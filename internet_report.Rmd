...
ranking <- function(df, var, plot_title = "",
  ...
  p <- p + theme(text = element_text(family = "Museo Sans 300"),
                 ...
                 axis.text.y = element_text(hjust = 0,
                                            size = 40 * text_size, color = rev(df$textcolor)),
...
plt_by <- function(df, group_var, title_text = "Internet Access") {
  ...
  plt <- plt  + 
    scale_colour_manual(values = c("#0e4a99", "#f58021")) +
...
ggplot(jeff_co_sf) + 
  ...
  geom_sf_label(aes(label = int_num), family = "Museo Sans 300", fontface = "bold", size = 8) +
